...
class SRGAN(object):
    ...
    def train(self):
        ...
        # test image
        test_lr, test_hr, test_bc = test_data_loader.dataset.__getitem__(1)
    ...
    def predict2d(self,test_dataset,output='pic'):
        ...
        # CMD
        image_filenames.extend(join(image_dir, x) for x in sorted(listdir(image_dir)) if utils.is_imagefile(x))
    ...
    def load_model(self, is_pretrain=False):
        ...
        if is_pretrain:
            ...
            if os.path.exists(model_name):
                ...
                pretrained_dict = {k: v for k, v in pretrained_dict.items() if 'input_conv' not in k and 'output_conv' not in k}
                # 2. overwrite entries in the existing state dict
                model_dict.update(pretrained_dict)
                self.G.load_state_dict(model_dict)
                print('Pre-trained generator model is loaded:'+model_name)
                return True
        else:
            ...
            if os.path.exists(model_name):
                self.G.load_state_dict(torch.load(model_name))
                # print('Trained generator model is loaded:'+model_name)
                return True
