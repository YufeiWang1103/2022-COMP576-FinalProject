...
def get_training_set(data_dir, dataset, crop_size, scale_factor,registered=False,grayscale_corrected=True):
    ...
    if registered:
        ...
        for data in dataset:
            ...
            train_dir.append(join(data_dir, data))
...
def get_test_set(data_dir, dataset,crop_size,scale_factor,registered=False,grayscale_corrected=True):
    ...
    if registered:
        ...
        for data in dataset:
            ...
            # CMD
            test_dir.append(join(data_dir, data))
        # pdb.set_trace()
...
class LoadTrainDataset_2d(data.Dataset):
    ...
    def __init__(self, image_dirs,scale=(0,1.0),random_scale=False, crop_size=100, rotate=True,fliplr=True, fliptb=True, scale_factor=4):
        ...
        for image_dir in image_dirs:
            ...
            self.image_filenames.extend(join(image_dir, x) for x in sorted(listdir(image_dir)) if is_imagefile(x))
    ...
    def __getitem__(self, index):  
        ...
        # img = img.filter(MyGaussianBlur(radius=1.5))
        # print(img_array.shape)  
        # print(self.crop_size)
        # # img = gaussian_filter(img, sigma=1.5, mode='nearest')
        # plt.figure("dog")
...
class LoadTestDataset_2d(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(img.size)
        # org_img = copy.deepcopy(img)
        # img = img.filter(MyGaussianBlur(radius=5))
